...
import { 
  ...
  Waves, ArrowLeft, Save, Loader2, Info, AlertTriangle, CheckCircle2, ChevronRight
} from "lucide-react"
...
const NadadorForm = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-24 p-4">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left">
              <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-tight">
                {isEdit ? "Editar" : "Nuevo"} <span className="bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent not-italic">Atleta</span>
              </h2>
      ...
      {serverError && (
        <div className="bg-orange-500 text-white p-6 rounded-[2rem] flex items-center gap-5 animate-bounce-subtle shadow-xl shadow-orange-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest px-4 py-2 bg-slate-50 rounded-full italic">Datos visibles en el perfil público del nadador</span>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-end gap-6 pt-10">
           ...
           >
             {mutation.isLoading ? <Loader2 size={24} className="animate-spin" /> : <CheckCircle2 size={24} />}
...
const CustomInput = ({ label, name, type = "text", icon: Icon, form, setForm, errors, placeholder, description, disabled, onChange }) => (
  <div className={`flex flex-col space-y-3 ${disabled ? 'opacity-70' : ''}`}>
    ...
    <div className="relative group">
      ...
      />
      {errors[name] && (
        <div className="absolute right-4 top-1/2 -translate-y-1/2 text-orange-500 animate-in fade-in zoom-in">
