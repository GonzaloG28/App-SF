...
import {
  ...
  ShieldCheck, RefreshCcw, User, Target, Zap, AlertCircle
} from "lucide-react"
...
const NadadorProfile = () => {
  ...
  const { data: nadador, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ["nadador", id],
    queryFn:  async () => (await (await import("../../api/profesor.api")).getNadadorById(id)).data,
    enabled:  !!id,
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 md:space-y-8 animate-fade-in pb-8">
      ...
      <div className="relative group overflow-hidden bg-white rounded-2xl md:rounded-[2.5rem] p-5 md:p-10 border border-slate-100 shadow-lg">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-5 md:gap-10">
          ...
          <div className="relative shrink-0">
            <div className="w-20 h-20 sm:w-28 sm:h-28 md:w-36 md:h-36 rounded-2xl md:rounded-[2rem] bg-gradient-to-br from-blue-600 to-green-500 flex items-center justify-center text-3xl md:text-5xl font-black italic text-white shadow-xl shadow-blue-600/20">
      ...
      {/* MÉTRICAS — 2 columnas en mobile, 4 en desktop */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-8">
        ...
        {/* ACCIONES — en mobile van en fila, en desktop en columna */}
...
const SidebarAction = ({ to, icon: Icon, title, subtitle, color }) => (
  <Link
    to={to}
    className="group flex items-center justify-between bg-white p-4 md:p-6 rounded-2xl border border-slate-100 shadow-sm hover:bg-slate-900 transition-all duration-500"
  >
    <div className="flex items-center gap-3">
      <div className={`p-2.5 md:p-3.5 rounded-xl transition-all group-hover:bg-white/10 group-hover:text-white ${color === "blue" ? "bg-blue-50 text-blue-600" : "bg-emerald-50 text-emerald-600"}`}>
        <Icon size={18} />
      </div>
...
// LOADING MEJORADO — skeleton animado en vez de spinner solo
const LoadingState = () => (
