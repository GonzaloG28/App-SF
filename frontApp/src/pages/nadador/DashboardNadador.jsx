    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-[3rem] p-8 md:p-12 max-w-lg w-full shadow-2xl relative overflow-hidden border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          />
          {error && (
            <p className="text-orange-600 text-[10px] font-black uppercase text-center italic">
              {error}
            </p>
          )}
          <button 
...
const DashboardNadador = () => {
  ...
  // FIX DE SEGURIDAD: El modal ya no se controla desde localStorage.
  // Antes: localStorage.getItem("debeCambiarPassword") === "true"
  //   → cualquier usuario podía abrir DevTools y escribir
  //     localStorage.setItem("debeCambiarPassword", "false") para saltarse el cambio.
  // Ahora: el valor viene del servidor a través de la query del perfil.
  ...
  // FIX: Abrimos el modal solo cuando el servidor confirma que debe cambiar clave.
  // perfil.user.debeCambiarPassword viene del endpoint /nadadores/perfil
  // que hace populate con el campo debeCambiarPassword del modelo User.
  useEffect(() => {
    if (perfil?.user?.debeCambiarPassword === true) {
      setIsModalOpen(true);
    }
  }, [perfil]);
            ...
            <div className="grid grid-cols-2 gap-4">
              <Link to="/nadador/perfil" className="bg-white p-6 rounded-[2.2rem] border border-slate-100 hover:border-blue-200 transition-all group shadow-sm">
                ...
                <p className="text-2xl font-black italic text-slate-900 leading-none">
                  {perfil?.altura}<span className="text-[10px] ml-1 text-blue-600 uppercase not-italic">cm</span>
                </p>
              </Link>
              <Link to="/nadador/perfil" className="bg-white p-6 rounded-[2.2rem] border border-slate-100 hover:border-green-200 transition-all group shadow-sm">
                ...
                <p className="text-2xl font-black italic text-slate-900 leading-none">
                  {perfil?.peso}<span className="text-[10px] ml-1 text-green-600 uppercase not-italic">kg</span>
                </p>
              </Link>
            </div>
          </div>
            ...
            {ultimoEntreno ? (
              <div>
                ...
                <div className="inline-flex items-center gap-3 bg-blue-50 px-4 py-2 rounded-xl border border-blue-100">
                  ...
                  <p className="text-[10px] font-black italic text-blue-800 uppercase tracking-widest">
