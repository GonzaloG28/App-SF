        ...
        <nav className="flex flex-col gap-1">
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              <ChevronRight 
                size={16} 
                className={`transition-transform duration-300 ${showEntrenamientos ? "rotate-90 text-blue-600" : "opacity-40"}`} 
              />
            </button>

            {showEntrenamientos && (
              <div className="flex flex-col ml-9 mt-1 gap-0.5 border-l-2 border-green-100 pl-4 animate-in slide-in-from-top-2 duration-300">
                <Link to="/profesor/crear-entrenamiento" className={`text-[10px] font-black uppercase tracking-widest py-2.5 transition-colors ${location.pathname === "/profesor/crear-entrenamiento" ? "text-green-600" : "text-slate-400 hover:text-blue-500"}`}>• Crear Nuevo</Link>
                <Link to="/profesor/entrenamientos" className={`text-[10px] font-black uppercase tracking-widest py-2.5 transition-colors ${location.pathname === "/profesor/entrenamientos" ? "text-green-600" : "text-slate-400 hover:text-blue-500"}`}>• Ver Reportes</Link>
              </div>
            )}
          </div>
    ...
    <div className="flex min-h-screen bg-[#FDFDFD] font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-20 px-6 lg:px-10 py-4 border-b border-slate-100 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            {/* --- SECCIÓN DE NOTIFICACIONES CON POPOVER --- */}
